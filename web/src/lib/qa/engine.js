// 知识问答 · 检索增强引擎（与 dsh-georag 共用实现）
// 三路召回：词法(BM25/中文二元组) + 图谱(双链扩展) + 空间(距离衰减)，融合排序。
...
export function createEngine({ nodes, days, mapLayers }) {
  // —— 节点索引、双链邻接 ——
  const nodeById = {}
  const titleToId = {}
  ...
  // —— 主检索 ——
  const W_DEFAULT = { lex: 1.0, graph: 0.5, spatial: 0.6 }
  ...
  function search(q, opt = {}) {
    ...
    for (const ci of cand) {
      ...
      let score = W.lex * sLex + W.graph * sGraph + (spatialActive ? W.spatial * sSpa : 0)
      if (wantsPoint && c.kind === '讲解点') score *= 1.25
      else if (wantsDef && (c.kind === '概念' || c.kind === '主线')) score *= 1.12
      if (score <= 0.02) continue
      scored.push({ chunk: c, score, sLex, sGraph, sSpa, dist })
    }
    ...
    return { hits, anchor, places: placeHits, spatialActive, seeds: [...seeds], locative }
  }
