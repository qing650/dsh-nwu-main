...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('keeps the settings content readable in a narrow viewport', async () => {
    const narrowPage = await browser.newPage({ viewport: { width: 275, height: 775 }, locale: ZH_BROWSER_LOCALE })
    const narrowTripwire = watchConsole(narrowPage)
    onTestFailed(() => saveFailureShot(narrowPage, 'web-e2e-settings-narrow'))
    try {
      await narrowPage.goto(scaffold.baseUrl, { waitUntil: 'load' })
      await narrowPage.waitForSelector('[class*="frame"]', { timeout: 30_000 })
      await narrowPage.locator('button[aria-haspopup="dialog"]').click()
      ...
      const generalBox = await dialog.getByRole('button', { name: '通用设置' }).boundingBox()
      const openDocumentBox = await openDocument.boundingBox()
      if (dialogBox === null || generalBox === null || openDocumentBox === null) {
        throw new Error('narrow settings controls have no layout boxes')
      }
      expect(dialogBox.width).toBeGreaterThan(240)
      expect(generalBox.width).toBeGreaterThan(100)
      expect(openDocumentBox.width).toBeGreaterThan(90)
      expect(openDocumentBox.y).toBeGreaterThan(generalBox.y + generalBox.height)
      expect(openDocumentBox.x + openDocumentBox.width).toBeLessThanOrEqual(dialogBox.x + dialogBox.width)
