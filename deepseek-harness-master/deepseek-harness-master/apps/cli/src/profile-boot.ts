...
function composeProfile(
  ...
  if (rows.has('agent-presets')) {
    const presetConfig = (rows.get('agent-presets')?.config ?? {}) as Record<string, unknown>
    composedOverlays.push({
      id: 'agent-presets',
      config: {
        ...presetConfig,
        // A deployment that explicitly owns its roster must be able to replace
        // the shipped coding modes. Only supply the CLI root when no layer has
        // declared `roots`; an explicit empty array is meaningful too.
        roots: Object.hasOwn(presetConfig, 'roots')
          ? presetConfig.roots
